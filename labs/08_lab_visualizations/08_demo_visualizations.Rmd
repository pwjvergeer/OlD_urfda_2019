...
```{r, echo=FALSE}
knitr::include_graphics(path = file.path(
  root, 
...
subset_gapminder <- dplyr::select(.data = gapminder, country, region)

table <- dplyr::select(.data = gapminder, -c(gdp, country, region))

readr::write_csv(table, 
                 path = file.path(
                   root, 
                   "data",
                   "gapminder_subset.csv")
                 )
...
ggsave(filename = "./images/bar_gapminder_facets.svg",
       dpi = 300, width = 20, height = 30, units = "cm")
