---
title: "Iterations: Loops and specialized functions"
author: "Marc A.T. Teunis"
date: "`r Sys.Date()`"
output: html_document
---

## Credits: partly reproduced and adapted from "R for Data Science" by Garret Grolemund & Hadley Whickham.
...
We talked about how important it is to reduce duplication in your code by creating functions instead of copying-and-pasting. Reducing code duplication has three main benefits:
...
Another tool for reducing duplication is __iteration__ (also called __loops__), which helps you when you need to do the same thing to multiple inputs: repeating the same operation on different columns, or on different datasets, or on different elements of a list or vector. 

In this chapter you'll learn about two important iteration paradigms: 

_Imperative programming_ and _functional programming_. 
...
## 1. Output
    The __output__: `output <- vector("double", length(x))`. 
...
## 3. Body
    The __body__: `output[[i]] <- median(df[[i]])`. This is the code that does
...
_Think about the output, sequence, and body **before** you start writing the loop_
...
```{r}
mtcars %>% length() ## determine how many columns
mtcars ## all variables are numeric

medians <- vector("numeric", 11) ## output

for(i in seq_along(mtcars)){     ## sequence
   medians[[i]] <- median(mtcars[[i]])  ## body
}
...
```{r}
x <- nycflights13::flights %>% length() ## determine how many columns

types <- vector("numeric", x)
...
```{r}
iris
x <- iris %>% length() ## determine how many columns

unique <- vector("numeric", x)

for(i in seq_along(iris)){
   unique[[i]] <- unique(iris[[i]]) %>% length() 
}
unique
```

D) Generate 10 random normals for each of $\mean = -10$, $0$, $10$, and $100$ and $sd = 1$. Collect the results in a list and after the loop is finished, convert this list to a draframe.

**TIPS**

 - remember: A dataframe needs column names
 - to create a dataframe from a list you can use `dplyr::bind_rows()` in this case
 - random normals can be generate with the function `rnorm`, look at the help for this function before writing the loop.
...
## a named vector to store the required means
means <- c(minus_ten = -10, zero = 0, ten = 10, hundred = 100)

for(i in seq_along(means)){
  
  normals[[i]] <- rnorm(n = 10, mean = means[i])

}

normals

## convert to dataframe, columns need a name
names(normals) <- names(means)
normals_df <- dplyr::bind_rows(normals)
normals_df
...
E) Rerun the previous loop (under D) but now generate 1000 random normals, generate a plot (using ggplot2) showing all 4 distributions using a frequency polynom. Display all lines in one graph, set standard diviation to 2.5
...
for(i in seq_along(means)){
  ...
  normals[[i]] <- rnorm(n = 1000, mean = means[i], sd = 2.5)
...
normals_df %>%
  ggplot2::ggplot() +
  geom_freqpoly(aes(x = minus_ten), colour = "red") +
  geom_freqpoly(aes(x = zero), colour = "blue") +
  geom_freqpoly(aes(x = ten), colour = "purple") +
  geom_freqpoly(aes(x = hundred), colour = "green")


```

## **EXERCISE 2**
A) Eliminate the for loop in the following example by taking advantage of an existing function that works with vectors:
...
2.  __Sequence__: we can think about a data frame as a list of columns, so 
    we can iterate over each column with `seq_along(df)`.

3.  __Body__: apply `rescale01()`.
...
Typically you'll be modifying a list or data frame with this sort of loop, so remember to use `[[`, not `[`. You might have spotted that we used `[[` in all my for loops: it's probably better to use `[[` even for atomic vectors because it makes it clear that we want to work with a single element (column in case of a dataframe).

### Looping patterns

There are three basic ways to loop over a vector. So far we have seen the most general: looping over the numeric indices with `for (i in seq_along(xs))`, and extracting the value with `x[[i]]`. There are two other forms:
...
While loops most often used for simulation. Although not within the scope of this course, it is good to know they exist so that you're prepared for problems where the number of iterations is not known in advance.

## **Exercise 3**

A) Imagine you have a directory full of CSV files that you want to read in.
...
B) Try the approach you wrote under A on the files in the zip file: "data/diet_data.zip". Unzip the zip-archive using R (`unzip()`). Generate a list of file paths using list.files(). Set the `full.names` option to `TRUE`.  
...
C) What happens if you use `for (nm in names(x))` and `x` has no nam
es?
...
**TIPS** 
 ...
 - generate test vectors with elements that: 1) have no names, 2) are partly named, 3) all have names.
