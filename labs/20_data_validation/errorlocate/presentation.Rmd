...
```{r, eval=TRUE}
x <- locate_errors( data.frame( age  = 3
                  , married = TRUE
                  , attends = "kindergarten"
                  )
...
```{r}
df <- data.frame(age     = 26, 
                 married = TRUE, 
                 attends = "kindergarten")

x <- locate_errors(data = df, x = rules)
x$errors # TRUE means this is an error, FALSE means this is not an error
```

b) Find the error with `locate_errors`: 

```{r}
df <- data.frame(age     = c(31,5), 
                 married = c(TRUE, TRUE), 
                 attends = c("kindergarten", "school")
)
...
c) Run `locate_errors` with the `rules_dl` (!) and add a weight of 2 to `age`. Use the drivers licence datset
