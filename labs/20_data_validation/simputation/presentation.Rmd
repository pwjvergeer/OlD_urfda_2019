...
data(retailers,package="validate")
ret_0 <- retailers
ret_1 <- ret_0 %>% impute_lm(other.rev ~ turnover) %>% head()

```


## Assignment 2: Try the following code
## use a nested `list-columns` approach to compare the imputations
```{r,results='hide'}
# note the 'rlm'!
ret_2 <- ret_0 %>% impute_rlm(other.rev ~ turnover) %>% head()


plot_imputation <- function(df){
...
plot_imputation(ret_2)

models <- c(impute_lm, impute_rlm) %>% as.list


collect <- ret_0 %>%
  tidyr::nest() %>%
  mutate(ret_1 = impute_lm(data, other.rev ~ turnover),
         ret_2 = impute_rlm(data, other.rev ~ turnover)
  ) %>% 
  gather(data:ret_2, key = "model", value = "data") %>%
  unnest(data)
