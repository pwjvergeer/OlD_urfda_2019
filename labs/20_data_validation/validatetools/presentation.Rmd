...
validatetools::detect_boundary_num(rules)
...
```{r}
rules <- validator(.file = "./validatetools/infeasible_rules.txt")

## detect problemes with rules
is_infeasible(rules)

x <- detect_infeasible_rules(rules)
# And its conflicting rule(s)
y <- is_contradicted_by(rules, "V017")
