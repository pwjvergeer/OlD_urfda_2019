...
head(retailers)
retailers %>%
  check_that(tot_rev = turnover + other.rev == total.rev, 
             turnover = turnover > 0, 
             other_rev = other.rev > 0 ) %>%
  summary()

## result is within bounds of tolerance (floting point issues)
...
# using this same  syntax works also for factors (use %in% )
...
out %>% plot()
# ?validate::confront
out %>% as.data.frame() %>% tibble::as_tibble()
